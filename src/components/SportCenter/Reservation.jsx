...
const Reservation = ({ show, onHide, field, loggedUser }) => {
  ...
  const fetchReservation = async () => {
    try {
      ...
      const response = await fetch(
        `${URL}/api/reservation/field/${field._id}`,
        {
          method: "GET",
          headers: {
  ...
  const filterReservedHours = () => {
    if (startDate) {
      ...
      for (let i = 0; i < todayReservations.length; i++) {
        reservatedHours.push(
          new Date(Date.parse(todayReservations[i].ReservationTime))
        );
      }
  ...
  const validacionFecha = () => {
    ...
    if (
      startDate.getFullYear < startTime.getFullYear() ||
      (startDate.getFullYear < startTime.getFullYear() &&
        startDate.getMonth() < startTime.getMonth()) ||
      (startDate.getFullYear < startTime.getFullYear() &&
        startDate.getMonth() < startTime.getMonth() &&
        startDate.getDate() < startTime.getDate()) ||
      startDate.getHours() < startTime.getHours()
    ) {
      return {
        error: true,
    ...
    if (
      ...
      startDate.getHours() > endTime.getHours()
    ) {
      return {
        error: true,
    ...
    if (
      startTime.getHours() > field.closeHour ||
      startTime.getHours() < field.openHour
    ) {
      return {
        error: true,
    ...
    if (filterReservedHours()) {
      const colission = filterReservedHours().find(
        (reservated) => startDate.getHours() === reservated.getHours()
      );
      if (colission) {
        return {
          error: true,
  ...
  const handleSubmit = (e) => {
    ...
    if (!errorFecha.error) {
      fetchPostReservation({
        ...
        IdField: field._id,
        ReservationTime: moment(startDate)
      ...
      >
        ...
        <Modal.Body>
              ...
              <Form.Group className="d-flex flex-column w-100">
                ...
                <DatePicker
                  ...
                  minDate={new Date().setDate(
                    addDay(new Date().getDate(), new Date().getMonth(), 1)
                  )}
                  maxDate={endDate}
                  excludeTimes={startDate ? filterReservedHours() : [startTime]}
                  timeCaption="Hora"
                  minTime={startTime}
                  maxTime={startDate ? endTime : startTime}
          ...
          <p className="text-muted my-2">
            Podes alquilar tu cancha entre 1 o 60 dias antes **
          </p>
        </Modal.Body>
      </Modal>
    </>
